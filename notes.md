...
- UI refinement: locked List columns, wider default window, wrapped Location paths, row-wide accent selection with neutral status pill backgrounds, and reliable Select All/Clear card styling. Status pill matches the 20px badge height without additive padding.
- Gallery short titles scale up within rows containing wrapped titles. Artwork accent strokes share 3px treatment; removed the dark artwork frame fill/shadow. Standalone button hairlines are subtle; linked controls remain seamless. Light accents, selected view controls and Notes surfaces improved.
- Progress uses a blurred, subtly grained hero only alongside a poster. Review has its own scrollable layout; result replaces the entire progress layout, fixing clipped errors and duplicate corner treatments. Complete descriptions are available in Overview; hero preview retained.
- Added bulk Update DLSS in Features using progress UI; install preset prompt explains later OptiScaler adjustment. Same-version verified replacement remains explicit and skips byte-identical files. Sidecar comparisons do not claim a current shipping baseline.
- Verified desktop smoke in all three themes, long error content, narrow preset/review dialogs, locked List columns, selection synchronization and 12 runtime fixture tests. Frame-generation investigation is excluded; metadata source expansion remains pending.
