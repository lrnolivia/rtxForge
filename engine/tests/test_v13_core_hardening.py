...
class CoreHardeningTests(unittest.TestCase):
    ...
    def test_dlss_unlocked_0_5_8_multiplier_sets_game_owned_override_only(self):
        self.game.dlss = self.game.dlssg = True
        self.m.Y4MY_PROVIDER = {**self.m.Y4MY_PROVIDER, 'id':'dlss-unlocked'}
        self.m.install_target(self.game,self._sm86_payload(),{'sha256':'a'*64},'ada',feature_mode='mfg-only',enable_effects=True,mfg_multiplier=4)
        text=(self.target/'OptiScaler.ini').read_text()
        self.assertIn('OverrideInterpolationCount=3',text)
        self.assertIn('InterpolationCount=auto',text)
        self.assertIn('FGOutput=auto',text)
